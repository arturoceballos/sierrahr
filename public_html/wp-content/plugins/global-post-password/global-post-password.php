...
class GlobalPostPassword {
	...
	function permalink_rss( $permalink ) {
		...
		if ( !$post->post_password )
			return $permalink;
		$permalink = add_query_arg( 'pass', htmlspecialchars( stripslashes( $_GET['pass'] ) ), $permalink );
		return str_replace( '&', '&amp;', $permalink );
	}

	function register() {
		register_setting( 'globalpostpassword', 'globalpostpassword', array( $this, 'update' ) );
		register_setting( 'globalpassids', 'globalpassids', array( $this, 'updateids' ) );
	...
	function buffer_edit( $content ) {
		...
		$replace = '<input type="hidden" name="post_password" value="' . htmlspecialchars( $passes[0] ) . '" />';
	...
	function js_manage() {
		...
		jQuery(document).ready(function($) {
			...
			$('<label><input type="checkbox" class="inline-edit-password-switch" /> <span class="checkbox-title"><?php _e('Password protected', 'g_p_p' ); ?></span></label>').insertAfter('.inline-edit-password-input').click(function(){
				val = $('input',this).attr('checked') ? '<?php echo htmlspecialchars( $passes[0] ); ?>' : '';
	...
	function js_privacy() {
		...
		jQuery(function($) {
			...
			if ( $('#g_p_p input').length > 1 )
				$('.postrow:last, .postrow br:last').hide();
			$('#g_p_p h4 a').click(function(){
				$('.postrow:last').after($('.postrow:last').clone().val('').show()).after('');
				return false;
			});
	...
	function settings() {
		$passes   = (array) get_option( 'globalpostpassword' );
		$settings = (array) get_option( 'globalpostpassword_settings' );
		$passids   = (array) get_option( 'globalpassids' );
		?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php _e('Global Post Password', 'g_p_p'); ?></th>
			<td><input name="globalpostpassword[0]" id="globalpostpassword" type="text" value="<?php echo htmlspecialchars( $passes[0] ); ?>" class="regular-text" maxlength="20" />
      ...
      $ip = 0;
        ...
        if ( !$key )
					continue;
				echo '<div class="postrow"><input name="globalpostpassword[]" type="text" value="' . htmlspecialchars( $pass ) . '" class="regular-text" maxlength="20" /> <input name="globalpassids[]" type="text" value="' . htmlspecialchars( $passids[$ip] ) . '" class="regular-text" maxlength="20" /></div>';
        $ip++;
      }

			?>
			<div class="postrow"><input name="globalpostpassword[]" type="text" value="" class="regular-text" maxlength="20" /> <input name="globalpassids[]" type="text" value="" class="regular-text" maxlength="20" /></div>
		...
		<tr valign="top">
			...
			<td>
				...
				<span class="setting-description description"><?php printf( _e( 'With this setting enabled, appending <code>?pass=%s</code> to a post permalink will display the post without asking for the password.', 'g_p_p' ), htmlspecialchars( $passes[0] ) ); ?></span></p>
				...
				<span class="setting-description description"><?php printf( _e( 'With this setting enabled, appending <code>?pass=%s</code> to your blog&rsquo;s feed URL will display password protected posts in your feed without asking for passwords.', 'g_p_p' ), htmlspecialchars( $passes[0] ) ); ?></span></p>
