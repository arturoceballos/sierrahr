...
function cforms($args = '',$no = '') {
	...
	global $myEvents, $smtpsettings, $subID, $cforms_root, $wpdb, $track, $wp_db_version, $cformsSettings;
  ...
  //echo $no;
  //echo "<pre>",print_r($cformsSettings['form'.$no]),"</pre>";
  //$myEvents->get_all_events();
  //$newvalues = $myEvents->create_admin_list('reserve');
  //echo $newvalues;
  $cformsSettings['form'.$no]['cforms_count_field_6'] = 'Eventman # first | first # second | second # third | third # fourth | fourth$#$selectbox$#$0$#$0$#$0$#$0$#$0';
  $cformsSettings['form'.$no]['cforms_count_field_6'] = $newvalues;
	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		if ( ! in_array($field_type,array('fieldsetstart','fieldsetend','radiobuttons','send2author','luv','subscribe','checkbox','checkboxgroup','ccbox','emailtobox','multiselectbox','verification')) ) {
      ...
      if($field_type == 'selectbox'){
        $arrayer = explode('#', $defaultvalue);
        $defaultvalue = $arrayer[0];
      }
    ...
    //echo $field_value;
		...
		switch($field_type) {
			...
			case "textarea":
				...
				$field = '<textarea' . $readonly.$disabled . ' cols="30" rows="3" name="'.$input_name.'" id="'.$input_id.'" class="' . $field_class . '"'. $onfocus.$fieldTitle.'>' . $field_value  . '</textarea>';
				  if ( $reg_exp<>'' )
	           		 $field .= '<input type="hidden" name="'.$input_name.'_regexp" id="'.$input_id.'_regexp" value="'.$reg_exp.'"'.$fieldTitle.'/>';
				break;
